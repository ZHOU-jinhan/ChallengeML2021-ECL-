...
def visualise_index(idx, data_h5, masks, method, N_signals=8):
    x = data_h5['data'][idx, 2:]
    x = x.reshape(N_signals, -1)
    visualize_signal_and_event(x, np.array(masks[idx, 1:]))
    plt.savefig(r"results/figures/"+method+"/"+method+"_"+str(idx)+".png")


if __name__ == "__main__":
    import h5py
    PATH_TO_TRAINING_DATA = "X_test.h5"
    method=input("Methode :\t")
    PATH_TO_TRAINING_TARGET = r"results/test_y/"+method+"_y.csv"
    h5_file = h5py.File(PATH_TO_TRAINING_DATA,"r")
    mask = np.array(pd.read_csv(PATH_TO_TRAINING_TARGET))
    for i in range(int(h5_file['data'].shape[0]/50)):
        visualise_index(i*50, h5_file, mask, method)
