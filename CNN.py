...
from tensorflow.keras.layers import Reshape,BatchNormalization,LeakyReLU,Dense,LSTM,Dropout,Conv2D,MaxPooling2D
import h5py
import numpy as np
import pandas as pd
...
class myCNN:
    ...
    def train(self,path_x='X_train.h5',path_y='Y_train.csv',epochs=30,batch_size=80,train_split=1,plot=1):
        ...
        if self.model==None:
            ...
            # Convolution layer
            self.model.add(Conv2D(32,[3,3], padding='same', kernel_initializer='he_normal',input_shape=(train_X.shape[-3],train_X.shape[-2], train_X.shape[-1]))) 
            self.model.add(BatchNormalization(momentum=0.75))
            self.model.add(MaxPooling2D([2,2],padding="same"))
            self.model.add(Conv2D(64,[3,3], padding='same', kernel_initializer='he_normal')) 
            self.model.add(BatchNormalization(momentum=0.75))
            self.model.add(MaxPooling2D([2,2],padding="same"))
            self.model.add(Conv2D(64,[3,3], padding='same', kernel_initializer='he_normal')) 
            self.model.add(BatchNormalization(momentum=0.75))
            ...
            self.model.compile(loss=tf.keras.losses.BinaryCrossentropy(), optimizer=tf.keras.optimizers.Adam(lr=0.2e-3))
            self.model.summary()
        # fit network
        if train_split!=1:
...
##        pd.DataFrame((test_y>self.d_threshold).astype("int32")).to_csv("results/test_y/CNN_y.csv")
    ...
    def save(self):
        ...
        """
        np.save("results/models/CNN/threshold.npy",self.d_threshold)
        self.model.save('results/models/CNN/cnn_.h5')

    def load(self):
        """
        Load self.model for channel.
        """
        self.d_threshold=np.load("results/models/CNN/threshold.npy")
        self.model = load_model('results/models/CNN/cnn_.h5')
        self.model.summary()
